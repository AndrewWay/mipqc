if(exist('image_cells','var'))
    if(~exist('cell_classes','var'))
        ...
        %Generate randomly ordered list of indices for accessing
        %image_cells
        random_cell_indices=randperm(nObservations);
...
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    for i=1:nObservations
        if(cell_classes(i,1)<0)
            cell_index=random_cell_indices(i);
            mat = cell2mat(image_cells(cell_index));
            figure(1),imshow(mat)
